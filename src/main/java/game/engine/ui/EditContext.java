...
public class EditContext {
     ...
     * If no editor is active this will activate the provided editor and return true.
    ...
    public boolean requestStartEdit(int entityId, EditableLabel editor, String initial) {
        ...
        if (currentEditor == editor && currentEntityId == entityId) return true;
    ...
    public boolean isEditing(int entityId) { return currentEntityId == entityId && currentEditor != null; }

    /** Focus the currently active editor so the user can finish it. */
    public void focusCurrentEditor() {
        if (currentEditor != null) currentEditor.requestFocus();
    }
}
