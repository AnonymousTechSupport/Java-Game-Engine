...
/**
 * Renders a single entity row in the hierarchy with expand/collapse and inline
 * renaming support via EditableLabel.
 */
public class EntityRow {
    ...
    public game.engine.ui.components.InlineEditor getEditable() {
    ...
    public void render(String displayName, Selection selection, List<ComponentType> components, Callback cb) {
        ...
        // Helper to draw the tree node arrow with transparent header (so ImGui
        // doesn't draw its hover box).
        ...
        if (editable.isEditing()) {
            ...
            if (centerOffset > 0)
                ImGui.setCursorPosY(currentY + centerOffset);
            ...
            if (ImGui.isItemHovered() && ImGui.isMouseDoubleClicked(0))
                editable.tryStart(entityId);
            if (ImGui.isItemClicked() && cb != null)
                cb.onSelect(new Selection(entityId));
        }

        if (nodeExpanded != this.expanded) {
            this.expanded = nodeExpanded;
            if (cb != null)
                cb.onToggleExpand(entityId, this.expanded);
        }

        contextMenu.render(uiContext, builder -> {
            // Use a submenu so the Add Component entry expands with the
            // triangle and
            // can show grouped Engine / Custom components (populated by the
            // helper).
        ...
        if (nodeExpanded) {
            for (ComponentType component : components) {
                boolean isComponentSelected = selection != null && selection.isComponent() && selection.getEntityId() == entityId
                ...
                if (childOffset != 0)
                    ImGui.setCursorPosY(childY + childOffset);
                ...
                if (ImGui.isItemClicked())
                    cb.onComponentSelected(entityId, component);
