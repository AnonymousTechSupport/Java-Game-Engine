...
/**
 * A UI panel that displays the list of entities in the scene hierarchy. It
 * allows selecting entities, renaming them, and deleting them. This panel
...
public class SceneHierarchyPanel extends UIComponentWithContext implements SelectionService.SelectionListener {
    ...
    public SceneHierarchyPanel(UIContext context, SelectionService selectionService, EditContext editContext, RenameService renameService) {
        super(context);
    ...
    public void render() {
        ...
        // Render each entity as a row in the hierarchy. We maintain a map of
        // entityId -> EntityRow to preserve
        // per-row UI state (like rename text and expanded/collapsed) across
        // frames.
        for (Entity e : entities) {
            int entityId = e.getId();
            String currentName = e.getName() != null ? e.getName() : ("Entity " + entityId);

            EntityRow row = rowMap.computeIfAbsent(entityId, id -> new EntityRow(id, currentName, editContext, uiContext));

            if (!row.isEditing())
                row.setText(currentName);
            ...
            row.render(currentName, currentSelection, components, new EntityRow.Callback() {
                ...
                @Override
                public void onRename(int id, String newName) {
                    // If newName==null it's a request to start editing from the
                    // menu
                    if (newName == null) {
                        // request start edit via EditContext using the row's
                        // editable
                        EntityRow r = rowMap.get(id);
                        if (r != null) {
                            boolean started = editContext.requestStartEdit(id, r.getEditable(), currentName);
                            if (started)
                                selectionService.setSelected(new Selection(id));
                        }
                    } else {
                ...
                @Override
                public void onToggleExpand(int id, boolean newState) {
                    expanded.put(id, newState);
                }
    ...
    // Render the "Add Component" context menu for a given entity, grouping
    // engine and custom components separately.
    ...
    // Handle deletion of an entity: cancel any active edits, enqueue the
    // destroy action, and clean up UI state.
    private void handleDeleteEntity(int entityId) {
        // Cancel any active edit immediately (UI thread) and enqueue destroy on
        // engine side.
