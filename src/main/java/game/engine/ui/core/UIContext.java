...
public class UIContext {
    ...
    /**
     * Defers an arbitrary action to be executed on the main/UI thread. This is useful for 
     * any operations that need to interact with the EntityRegistry or ECS in a thread-safe manner. 
     * For specific entity/component mutations, 
    ...
    /**
     * Returns a map of component type to component instance for the specified entity. 
     * This is a read-only operation and should be used for
     * UI rendering and display purposes. For mutation operations (adding/removing components),
     * use the provided defer* methods to ensure thread safety.
     * @param entityId
     * @return a Map of ComponentType to Component instance for the specified entity, or an empty map if the entity has no components.
     */
    ...
    /**
     * Returns the component of the specified type for the given entity, or null if the entity does not have that component. 
     * This is a read-only operation and should be used for 
     * UI rendering and display purposes. For mutation operations (adding/removing components), 
     * use the provided defer* methods to ensure thread safety.
     * @param entityId
     * @param type
     * @return the Component instance of the specified type for the given entity, or null if not present.
     */
    ...
    /**
     * Returns the list of available component types that can be added to entities. This is used to populate
     * the "Add Component" menu in the UI. INTERNAL component types (used for engine metadata and not user-facing) are filtered out.
    ...
    public ComponentType[] getAvailableComponentTypes() {
        ...
        return java.util.Arrays.stream(all)
                .filter(t -> t.getCategory() != game.engine.ECS.components.ComponentType.Category.INTERNAL)
                .toArray(ComponentType[]::new);
    }

    /**
     * Defers adding a component to an entity. The actual addComponent call will be executed on the main
     * thread to ensure thread safety with the underlying EntityRegistry and ECS.
     * @param entityId
     * @param type
     */
    ...
    /**
     * Defers removing a component from an entity. The actual removeComponent call will be executed on the main
     * thread to ensure thread safety with the underlying EntityRegistry and ECS.
     * @param entityId
     * @param type
     */
    ...
    /**
     * Defers destroying an entity. The actual destroyEntity call will be executed on the main
     * thread to ensure thread safety with the underlying EntityRegistry and ECS.
     * @param entityId
     */
